...
                <div className={`flex-shrink-0 h-9 w-9 rounded-xl flex items-center justify-center shadow-md ${isUser ? 'bg-gradient-to-tr from-primary to-indigo-600 text-white' : 'bg-gradient-to-tr from-emerald-600 to-teal-500 text-white'}`}>
                  {isUser ? <User size={16} /> : <Bot size={16} />}
                </div>
                <div className={`p-4 rounded-2xl shadow-sm text-left leading-relaxed relative group ${isUser ? 'bg-primary text-white rounded-tr-none' : 'bg-white/80 dark:bg-gray-900/60 backdrop-blur-md text-gray-900 dark:text-gray-150 rounded-tl-none border border-gray-150 dark:border-gray-800'}`}>
                  {!isUser && (
                    <button onClick={() => handleCopy(msg.text, msg.id)} className="absolute top-2 right-2 p-1 text-gray-400 hover:text-primary transition-colors opacity-0 group-hover:opacity-100" title="Copy to clipboard">
                      {copiedId === msg.id ? <Check size={14} /> : <Copy size={14} />}
                    </button>
                  )}
                  <p className="text-sm font-medium whitespace-pre-wrap">{msg.text}</p>
                  <p className={`text-[9px] font-semibold mt-2 ${isUser ? 'text-blue-100 text-right' : 'text-gray-400 dark:text-gray-500'}`}>{msg.time}</p>
                </div>
              </div>
            </div>
          );
